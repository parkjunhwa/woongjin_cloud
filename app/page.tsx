...
const getRadioColorClasses = (color: string) => {
  const c = colorClassMap[color] || "blue";
  return {
    hoverBorder: `hover:border-${c}-500 dark:hover:border-${c}-500`,
    indicatorBg: `bg-${c}-500 dark:bg-${c}-600`,
  };
};
...
const getCheckboxColorClasses = (color: string) => {
  const c = colorClassMap[color] || "blue";
  return {
    checkedBorder: `border-${c}-500 dark:border-${c}-600`,
    indicatorText: `text-${c}-500 dark:text-${c}-400`,
  };
};

const getRadioCardColorClasses = (color: string) => {
  const c = colorClassMap[color] || "blue";
  return {
    cardBorder: `border-${c}-500 dark:border-${c}-600`,
    cardBg: `bg-${c}-50 dark:bg-${c}-900/20`,
    indicatorBg: `bg-${c}-500 dark:bg-${c}-600`,
    text: `text-${c}-900 dark:text-${c}-100`,
  };
};

const getCheckboxCardColorClasses = (color: string) => {
  const c = colorClassMap[color] || "blue";
  return {
    cardBorder: `border-${c}-500 dark:border-${c}-600`,
    cardBg: `bg-${c}-50 dark:bg-${c}-900/20`,
    checkboxCheckedBg: `data-[state=checked]:bg-${c}-500`,
    checkboxCheckedBorder: `border-${c}-500 dark:border-${c}-600`,
    checkboxIndicatorText: `text-${c}-500 dark:text-${c}-400`,
    text: `text-${c}-900 dark:text-${c}-100`,
  };
};

const getSwitchColorClasses = (color: string) => {
  const c = colorClassMap[color] || "blue";
  return {
    checkedBg: `data-[state=checked]:bg-${c}-500 dark:data-[state=checked]:bg-${c}-600`,
  };
};

const getToggleGroupColorClasses = (color: string) => {
  const c = colorClassMap[color] || "blue";
  return {
    onBg: `data-[state=on]:bg-${c}-500 dark:data-[state=on]:bg-${c}-600`,
  };
};

const getTabsColorClasses = (color: string) => {
  const c = colorClassMap[color] || "blue";
  return {
    activeBorder: `data-[state=active]:border-${c}-500 dark:data-[state=active]:border-${c}-600`,
    activeText: `data-[state=active]:text-${c}-600 dark:data-[state=active]:text-${c}-400`,
  };
};
